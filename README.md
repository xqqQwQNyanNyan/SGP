...
`agent.py` 会读取 `agent.config.json`，再按其中的 `services_config` 发布服务。没有配置 `services_config` 时，Agent 只登录 Relay，不会默认发布 `note-box`、`win-command` 或 `frontend-workspace`。

每个 Service 的具体能力由 `services.json` 中的 `handler.path` 决定，协议和 Agent 主流程不写死 `text.echo`、`command.exec` 或 `http.bundle` 的业务逻辑。
...
```bash
python3 agent.py --services-config services.json
```

配置文件中的每个 Service 都会被发布到 Relay。Service 通过本地 handler 文件提供具体能力。
